...
export function DeviceControllerFactory(container: Container, apiVersion: number): interfaces.Controller {
  ...
  class DeviceController extends BaseHttpController {
    ...
    @createMethod
    private async retrieveQrCode(@requestBody() { imei }: { imei: string}): Promise<QrCode> {
      const qrCode = await this.lteService.retrieveQrCode(imei);
      return {
        qrCode
      };
    }
